...
4. Спрашивает параметры: hostname PMG, адрес отправителя, порт action-сервера
   > ⚠️ **Адрес отправителя** должен быть реальным и разрешённым на вашем почтовом сервере.
   > Несуществующий или заблокированный адрес вызовет ошибку `5.7.1 Sender address rejected`.
   > Используйте, например, `noreply@ваш-домен.com`.
...
### Исключение адресов из уведомлений

Иногда нужно не отправлять уведомления на определённые ящики — служебные адреса, роботы, общие ящики, которые никто не читает. Для этого в `[notifications]` есть параметр `exclude_addresses`:

```ini
[notifications]
exclude_addresses = robot@example.com, noreply@example.com, shared@example.com
```
