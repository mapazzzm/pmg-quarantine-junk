...
### База данных PMG в кодировке SQL_ASCII

PostgreSQL в PMG работает с кодировкой `SQL_ASCII`. При ручной вставке тестовых записей в `CMailStore` поле `header` должно содержать **только ASCII-символы** — кириллица и любые не-ASCII символы вызовут ошибку psycopg2:

```
UnicodeDecodeError: 'ascii' codec can't decode byte 0xd0...
```

Реальный PMG хранит заголовки в формате RFC 2047 (`=?UTF-8?B?...?=` или `=?UTF-8?Q?...?=`).
